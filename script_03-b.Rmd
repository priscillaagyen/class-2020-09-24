...
```{r}
sep_diff <-
inner_join(sep_old, sep_new, by = "id", suffix = c("_old","")) %>%
mutate(enrollment_diff =u_grad - u_grad_old) %>%
select("id", "title", "name", "department", "u_grad_old", "u_grad", "enrollment_diff") %>%
drop_na()%>%
  arrange(desc(enrollment_diff)) %>%
slice(1:5)
sep_diff %>%
  gt()
```
...
```{r}
sep_dif_total <- full_join(sep_diff, sep_diff1) %>%
 ggplot(aes(x= title, y = enrollment_diff)) +
 geom_col(aes(fill = department), color = "white") +
